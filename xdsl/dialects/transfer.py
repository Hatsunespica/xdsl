...
@irdl_attr_definition
class AbstractValueType(ParametrizedAttribute):
    name: str = "absValue"

    fields: ParameterDef[ArrayAttr[IntegerType]]

    def get_num_fields(self) -> int:
        return len(self.fields.data)

    def get_fields(self) -> List[IntegerType]:
        return [i for i in self.fields.data]

    def __init__(self, shape: list[IntegerType] | ArrayAttr[IntegerType]) -> None:
...
class GetOp(IRDLOperation):
    ...
    result: Annotated[OpResult, ContainerOf(IntegerType)]

    def verify_(self) -> None:
        if len(self.operands) != 2 or len(self.results) != 1:
            raise VerifyException("GET operation expects 3 operands and 1 result.")
